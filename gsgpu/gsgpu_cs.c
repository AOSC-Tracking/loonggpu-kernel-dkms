					      ...
					      u64 *max_vis_bytes)
{
	...
	used_vram = gsgpu_vram_mgr_usage(lg_bdev_to_ttm_man(&adev->mman.bdev, TTM_PL_VRAM));
	...
	*max_bytes = us_to_bytes(adev, adev->mm_stats.accum_us);
	...
	if (!gsgpu_gmc_vram_full_visible(&adev->gmc)) {
		...
		u64 used_vis_vram =
			gsgpu_vram_mgr_vis_usage(lg_bdev_to_ttm_man(&adev->mman.bdev, TTM_PL_VRAM));
				...
				struct gsgpu_bo *validated)
{
	...
	for (; &p->evictable->tv.head != &p->validated;
		...
		other = gsgpu_mem_type_to_domain(lg_get_bo_mem_type(bo));
...
static int gsgpu_cs_sync_rings(struct gsgpu_cs_parser *p)
{
	...
	list_for_each_entry(e, &p->validated, tv.head) {
		...
		r = gsgpu_sync_resv(p->adev, &p->job->sync, resv, GSGPU_SYNC_ALWAYS, p->filp,
					    ...
					    struct gsgpu_cs_chunk *chunk)
{
	...
	for (i = 0; i < num_deps; ++i) {
		r = gsgpu_syncobj_lookup_and_add_to_sync(p, deps[i].handle, 0, 0);
		if (r)
			return r;
	}
	return 0;
...
static int gsgpu_cs_process_syncobj_timeline_in_dep(struct gsgpu_cs_parser *p,
						     struct gsgpu_cs_chunk *chunk)
{
	struct drm_gsgpu_cs_chunk_syncobj *syncobj_deps;
	unsigned num_deps;
	int i, r;

	syncobj_deps = (struct drm_gsgpu_cs_chunk_syncobj *)chunk->kdata;
	num_deps = chunk->length_dw * 4 /
		sizeof(struct drm_gsgpu_cs_chunk_syncobj);
	for (i = 0; i < num_deps; ++i) {
		r = gsgpu_syncobj_lookup_and_add_to_sync(p,
...
static int gsgpu_cs_process_syncobj_timeline_out_dep(struct gsgpu_cs_parser *p,
...
static int gsgpu_cs_prepare_dependencies(struct gsgpu_cs_parser *p)
{
	int i, r;
	...
	for (i = 0; i < p->nchunks; ++i) {
		...
		if (chunk->chunk_id == GSGPU_CHUNK_ID_SYNCOBJ_TIMELINE_WAIT) {
			r = gsgpu_cs_process_syncobj_timeline_in_dep(p, chunk);
			if (r)
				return r;
		} else if (chunk->chunk_id == GSGPU_CHUNK_ID_SYNCOBJ_TIMELINE_SIGNAL) {
			r = gsgpu_cs_process_syncobj_timeline_out_dep(p, chunk);
			if (r)
				return r;
		}
	}
...
int gsgpu_cs_ioctl(struct drm_device *dev, void *data, struct drm_file *filp)
{
	...
	r = gsgpu_cs_prepare_dependencies(&parser);
	if (r)
		goto out;
