 ...
 */
static unsigned int gsgpu_device_vga_set_decode(lg_vga_set_decode_args)
{
	struct gsgpu_device *adev = lg_vga_set_decode_get_adev;
		       ...
		       uint32_t flags)
{
	 ...
	 * ignore it */
	lg_vga_client_register(adev->loongson_dc, adev, NULL, gsgpu_device_vga_set_decode);
...
retry:
		...
		if (!r) {
			...
			r = gsgpu_gtt_mgr_recover(lg_bdev_to_ttm_man(&adev->mman.bdev, TTM_PL_TT));
			if (r)
				goto out;
