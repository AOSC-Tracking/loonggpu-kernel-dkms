...
static void check_hdmi_support_audio(struct gsgpu_device *adev, struct drm_connector *connector, struct edid *edid)
{
	u8 *ext_edid = NULL;
	bool audio_support;
	u32 val;
	...
	if (ext_edid && audio_support) {
		gsgpu_hdmi_audio_init(adev, connector->index);
		DRM_DEBUG("This monitor supports audio and turns on hdmi audio. \n");
	} else {
		val = dc_readl(adev, CURRENT_REG(DC_HDMI_CTRL_REG, connector->index));
		dc_writel(adev, CURRENT_REG(DC_HDMI_CTRL_REG, connector->index), 0x288 | (val & 1));
...
static int gsgpu_dc_connector_get_modes(struct drm_connector *connector)
{
	...
	if (edid) {
		...
		check_hdmi_support_audio(lgdev, connector, edid);
...
static const struct drm_connector_funcs gsgpu_dc_connector_funcs = {
	...
	.late_register = gsgpu_late_register
};
...
int gsgpu_dc_connector_init(struct gsgpu_device *adev, uint32_t link_index)
{
	...
	case dev_7a2000:
		...
		if (dc_connector->resource->type == DRM_MODE_CONNECTOR_VGA &&
		    dc_connector->resource->hotplug == IRQ)
