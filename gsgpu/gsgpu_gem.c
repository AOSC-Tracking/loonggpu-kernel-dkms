			   ...
			   struct drm_file *file_priv)
{
	...
	if (abo->flags & GSGPU_GEM_CREATE_VM_ALWAYS_VALID && bo_resv != root_resv)
		return -EPERM;
			  ...
			  struct drm_file *filp)
{
	...
	if ((args->operation != GSGPU_VA_OP_CLEAR) &&
		...
		lg_ttm_validate_buffer_set_shared(&tv,
				!!(abo->flags & GSGPU_GEM_CREATE_VM_ALWAYS_VALID));
		list_add(&tv.head, &list);
	} else {
		gobj = NULL;
		abo = NULL;
...
static int gsgpu_debugfs_gem_bo_info(int id, void *ptr, void *data)
{
	...
	domain = gsgpu_mem_type_to_domain(lg_get_bo_mem_type(bo));
	switch (domain) {
