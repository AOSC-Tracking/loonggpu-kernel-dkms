...
u64 gsgpu_bo_gpu_offset(struct gsgpu_bo *bo)
{
	...
	WARN_ON_ONCE(lg_tbo_to_mem(&bo->tbo)->mem_type == TTM_PL_TT &&
		     !gsgpu_gtt_mgr_has_gart_addr(lg_tbo_to_mem(&bo->tbo)));
	WARN_ON_ONCE(!ww_mutex_is_locked(&resv->lock) && !lg_bo_pin_count(bo));
	WARN_ON_ONCE(lg_tbo_to_mem(&bo->tbo)->start == GSGPU_BO_INVALID_OFFSET);
	WARN_ON_ONCE(lg_tbo_to_mem(&bo->tbo)->mem_type == TTM_PL_VRAM &&
		     !(bo->flags & GSGPU_GEM_CREATE_VRAM_CONTIGUOUS));

	return lg_gsgpu_ttm_alloc_gart_get_bo_offset(adev, &bo->tbo);
}
