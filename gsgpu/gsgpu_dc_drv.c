...
extern bool gsgpu_resume;
extern bool gsgpu_suspend;
...
void dc_writel_check(struct gsgpu_device *adev, u32 reg, u32 val)
{
	...
	if (val_reg != val)
		DRM_INFO("GSGPU DC: write reg %x failed, target value is %x, but read-back value is %x\n", reg, val, val_reg);
}
...
static void gsgpu_output_poll_changed(struct drm_device *dev)
{
       struct gsgpu_device *adev = dev->dev_private;
       int i;

       for (i = 0; i < 2; i++) {
               struct gsgpu_dc_crtc *dc_crtc = adev->dc->link_info[i].crtc;
...
static const struct drm_mode_config_funcs gsgpu_dc_mode_funcs = {
	...
	lg_drm_mode_cfg_func_set_output_pull_changed
...
static void gsgpu_dc_atomic_commit_tail(struct drm_atomic_state *state)
{
	...
	for_each_oldnew_crtc_in_state(state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		if (old_crtc_state->active && !new_crtc_state->active) {
			if (drm_atomic_crtc_needs_modeset(new_crtc_state)) {
				dc_crtc_enable(to_gsgpu_crtc(crtc), false);
				gsgpu_hdmi_enable(adev, acrtc->crtc_id, false);
			}
		}
	...
	for_each_new_crtc_in_state(state, crtc, new_crtc_state, j) {
		...
		if (!new_crtc_state->active) {
			...
			gsgpu_hdmi_enable(adev, acrtc->crtc_id, true);
		}
...
static int dc_resume(void *handle)
{
	...
	for_each_oldnew_plane_in_state(dc->cached_state, plane, old_plane_state, new_plane_state, i) {
		...
		if (!fb)
			continue;
		afb = to_gsgpu_framebuffer(fb);
		if (!afb)
			continue;
		abo = gem_to_gsgpu_bo(fb->obj[0]);
		if (!abo)
			continue;
		kptr = gsgpu_bo_kptr(abo);
		if (!kptr)
			continue;
