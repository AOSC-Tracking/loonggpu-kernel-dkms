...
/**
 * gsgpu_ring_max_ibs - Return max IBs that fit in a single submission.
 *
 * @type: ring type for which to return the limit.
 */
static unsigned int gsgpu_ring_max_ibs(struct gsgpu_device *adev, enum gsgpu_ring_type type)
{
...
static int gsgpu_info_ioctl(struct drm_device *dev, void *data, struct drm_file *filp)
{
	...
	case GSGPU_INFO_VRAM_USAGE:
		ui64 = gsgpu_vram_mgr_usage(lg_bdev_to_ttm_man(&adev->mman.bdev, TTM_PL_VRAM));
		return copy_to_user(out, &ui64, min(size, 8u)) ? -EFAULT : 0;
	case GSGPU_INFO_VIS_VRAM_USAGE:
		ui64 = gsgpu_vram_mgr_vis_usage(lg_bdev_to_ttm_man(&adev->mman.bdev, TTM_PL_VRAM));
		return copy_to_user(out, &ui64, min(size, 8u)) ? -EFAULT : 0;
	case GSGPU_INFO_GTT_USAGE:
		ui64 = gsgpu_gtt_mgr_usage(lg_bdev_to_ttm_man(&adev->mman.bdev, TTM_PL_TT));
		return copy_to_user(out, &ui64, min(size, 8u)) ? -EFAULT : 0;
	...
	case GSGPU_INFO_MEMORY: {
		...
		mem.vram.usable_heap_size = adev->gmc.real_vram_size -
			atomic64_read(&adev->vram_pin_size);
		mem.vram.heap_usage =
			gsgpu_vram_mgr_usage(lg_bdev_to_ttm_man(&adev->mman.bdev, TTM_PL_VRAM));
		...
		mem.gtt.heap_usage =
			gsgpu_gtt_mgr_usage(lg_bdev_to_ttm_man(&adev->mman.bdev, TTM_PL_TT));
	...
	case GSGPU_INFO_VRAM_LOST_COUNTER:
		...
		return copy_to_user(out, &ui32, min(size, 4u)) ? -EFAULT : 0;
	case GSGPU_INFO_MAX_IBS: {
		uint32_t max_ibs[GSGPU_HW_IP_NUM];

		for (i = 0; i < GSGPU_HW_IP_NUM; ++i)
			max_ibs[i] = gsgpu_ring_max_ibs(adev, i);
				 ...
				 struct drm_file *file_priv)
{
	...
	if (pasid) {
		resv = to_dma_resv(pd);
		gsgpu_pasid_free_delayed(resv, pasid);
	}
	gsgpu_bo_unref(&pd);
