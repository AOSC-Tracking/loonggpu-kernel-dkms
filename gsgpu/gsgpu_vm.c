				   ...
				   struct gsgpu_bo *bo)
{
	...
	if (bo->preferred_domains &
	    gsgpu_mem_type_to_domain(lg_get_bo_mem_type(bo)))
		return;
			      ...
			      unsigned level)
{
	...
	r = gsgpu_sync_resv(ldev, &job->sync, resv, GSGPU_SYNC_ALWAYS,
			     ...
			     void *owner)
{
	...
	gsgpu_sync_resv(ldev, &sync, resv, GSGPU_SYNC_ALWAYS, owner, false);
				       ...
				       struct dma_fence **fence)
{
	...
	r = gsgpu_sync_resv(ldev, &job->sync, resv, GSGPU_SYNC_ALWAYS, owner, false);
	if (r)
		goto error_free;
				      ...
				      struct dma_fence **fence)
{
	...
	gsgpu_res_first(pages_addr ? NULL : res, mapping->offset,
			(mapping->last - mapping->start + 1) * GSGPU_GPU_PAGE_SIZE, &cursor);
	...
	while (cursor.remaining) {
		...
		if (pages_addr) {
			...
			max_entries = cursor.size / GSGPU_GPU_PAGE_SIZE;
			dma_addr = NULL;
		} else {
			addr = 0;
			dma_addr = NULL;
		...
		gsgpu_res_next(&cursor, (last - start + 1 ) * GSGPU_GPU_PAGE_SIZE);
			...
			bool clear)
{
	...
	list_for_each_entry(mapping, &bo_va->invalids, list) {
		r = gsgpu_vm_bo_split_mapping(ldev, mem, exclusive, pages_addr, vm,
			   ...
			   struct gsgpu_vm *vm)
{
	...
	list_for_each_entry_safe(bo_va, tmp, &moved, base.vm_status) {
		...
		else if (!gsgpu_vm_debug && lg_dma_resv_trylock(resv))
			clear = false;
...
void gsgpu_vm_bo_trace_cs(struct gsgpu_vm *vm, struct ww_acquire_ctx *ticket)
{
	...
	for (mapping = gsgpu_vm_it_iter_first(&vm->va, 0, U64_MAX); mapping;
		...
		if (mapping->bo_va && mapping->bo_va->base.bo) {
			struct gsgpu_bo *bo = mapping->bo_va->base.bo;
			lg_dma_resv_t *resv = to_dma_resv(bo);
