 ...
 */
static bool ncs8805_read_bytes(struct loonggpu_bridge_phy *phy, uint8_t addr,
...
/*Function as i2c_master_send */
static bool ncs8805_write_bytes(struct loonggpu_bridge_phy *phy, uint8_t addr,
...
static void ncs8805_rx_timing_get(struct loonggpu_bridge_phy *phy,
...
static int ncs8805_rx_timing_wait(struct loonggpu_bridge_phy *phy)
{
	struct rx_timing timing;
...
static int ncs8805_auto_mvid_detect(struct loonggpu_bridge_phy *phy)
{
	int index, ret = 0;
...
static int ncs8805_dp_lock_wait(struct loonggpu_bridge_phy *phy)
{
	int ret = 0;
	uint8_t reg_f, reg_g;
	uint8_t times_active = 10;
...
static int ncs8805_tx_timing_wait(struct loonggpu_bridge_phy *phy)
{
...
static void ncs8805_working_ctrl(struct loonggpu_bridge_phy *phy, uint8_t enable)
{
	uint8_t addr, reg;
	bool ret = false;
	...
	if (enable == NCS8805_WORKING_DISABLE) {
		...
		if (!ls_bl) {
			DRM_INFO("[%s] backlight not register,so working mode not set.\n",
					PHY_NAME);
			return;
		} else {
...
static bool ncs8805_mode_config(struct loonggpu_bridge_phy *phy,
...
static int ncs8805_bl_ctrl(struct loonggpu_bridge_phy *phy, int mode)
{
	struct loonggpu_backlight *ls_bl = NULL;
	ls_bl =  phy->adev->mode_info.backlights[1];
	...
	if (mode == DRM_MODE_DPMS_ON) {
		...
		/* Edp power on */
		loonggpu_lcd_ctrl_hw_action(ls_bl, true);
...
static int ncs8805_mode_set(struct loonggpu_bridge_phy *phy,
...
int ncs8805_suspend(struct loonggpu_bridge_phy *phy)
{
	ncs8805_bl_ctrl(phy, DRM_MODE_DPMS_OFF);
	return 0;
}

int ncs8805_resume(struct loonggpu_bridge_phy *phy)
{
...
static int ncs8805_get_modes(struct loonggpu_bridge_phy *phy,
...
{
	if (loonggpu_vbios_checksum(encoder_res->data, encoder_res->data_size)
			== encoder_res->data_checksum)
		return true;
	return false;
}

static enum hpd_status ncs8805_get_hpd_status(struct loonggpu_bridge_phy *phy)
{
       return hpd_status_plug_on;
}

static struct bridge_phy_hpd_funcs ncs8805_hpd_funcs = {
       .get_hpd_status = ncs8805_get_hpd_status,
};

int bridge_phy_ncs8805_init(struct loonggpu_dc_bridge *dc_bridge)
{
	struct loonggpu_bridge_phy *ncs8805_phy;
	struct ext_encoder_resources  *ext_resource;
	int ret = -1;
	int index;

	index = dc_bridge->display_pipe_index;
	if (loonggpu_vbios_version(dc_bridge->dc->vbios) >= VBIOS_VERSION_V1_1) {
		legacy_handle_flags = false;
		ext_resource = dc_get_vbios_resource(dc_bridge->dc->vbios, index, LOONGGPU_RESOURCE_EXT_ENCODER);
		if (!ext_resource) {
			DRM_WARN("Please check the resource information or display link information for NCS8805. \n");
			return ret;
		}
	...
	ncs8805_phy->is_ext_encoder = true;
	ncs8805_phy->cfg_funcs = &ncs8805_cfg_funcs;
	ncs8805_phy->ddc_funcs = &ncs8805_ddc_funcs;
	ncs8805_phy->hpd_funcs = &ncs8805_hpd_funcs;
	ncs8805_phy->regmap_cfg = NULL;

	return bridge_phy_init(ncs8805_phy);
}
