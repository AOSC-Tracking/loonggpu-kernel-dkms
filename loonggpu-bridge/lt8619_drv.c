...
static void lt8619_read_revision(struct loonggpu_bridge_phy *phy)
{
...
static int lt8619_config_rgb888_phy(struct loonggpu_bridge_phy *phy, bool enable)
{
	int ret;
	unsigned int val;
...
static void lt8619_edid_shadow_clk_enable(struct loonggpu_bridge_phy *phy)
{
	unsigned int val;
...
static void lt8619_lvds_tx_clk_enable(struct loonggpu_bridge_phy *phy)
{
	regmap_update_bits(phy->phy_regmap, 0x6006,
			   LVDS_TX_CLK_EN_BIT,
			   LVDS_TX_CLK_EN_BIT);
...
static int lt8619_enable_sys_clk(struct loonggpu_bridge_phy *phy, bool is_lvds)
{
...
static int lt8619_enable_cdr_bandwidth_adaptation(struct loonggpu_bridge_phy *phy)
{
	int ret;
...
static void lt8619_wait_rx_pll_locked(struct loonggpu_bridge_phy *phy)
{
	unsigned int val = 0;
 ...
 */
static void lt8619_wait_hdmi_stable(struct loonggpu_bridge_phy *phy)
{
	unsigned int val = 0;
	unsigned int count = 0;
...
static void lt8619_read_hdmi_clock_frequency(struct loonggpu_bridge_phy *phy,
					     unsigned int *pfreq)
{
...
static void lt8619_lvds_tx_ctl_soft_reset(struct loonggpu_bridge_phy *phy)
{
...
static void lt8619_edid_shadow_soft_reset(struct loonggpu_bridge_phy *phy)
{
...
static void lt8619_vid_chk_soft_reset(struct loonggpu_bridge_phy *phy)
{
 ...
 */
static ssize_t lt8619_read_hdmi_timings(struct loonggpu_bridge_phy *phy,
...
static void lt8619_turn_on_lvds(struct loonggpu_bridge_phy *phy)
{
...
static void lt8619_turn_off_lvds(struct loonggpu_bridge_phy *phy)
{
...
static void lt8619_hdmi_rx_reset(struct loonggpu_bridge_phy *phy)
{
...
static void lt8619_lvds_pll_lock_ctl_soft_reset(struct loonggpu_bridge_phy *phy)
{
...
/* LVDS PLL Soft Reset */
static void lt8619_lvds_pll_soft_reset(struct loonggpu_bridge_phy *phy)
{
...
static void lt8619_wait_lvds_pll_locked(struct loonggpu_bridge_phy *phy)
{
	unsigned int val = 0;
	unsigned int val1 = 0;
	unsigned int count = 0;
...
static void lt8619_config_lvds_pll(struct loonggpu_bridge_phy *phy)
{
...
static void lt8619_config_color_space(struct loonggpu_bridge_phy *phy)
{
...
static int lt8619_lvds_config(struct loonggpu_bridge_phy *phy)
{
	int ret;
...
static void lt8619_bridge_enable(struct loonggpu_bridge_phy *phy)
{
...
static int lt8619_reg_init(struct loonggpu_bridge_phy *phy)
{
...
static enum hpd_status lt8619_get_hpd_status(struct loonggpu_bridge_phy *phy)
{
       return hpd_status_plug_on;
}

static struct bridge_phy_hpd_funcs lt8619_hpd_funcs = {
       .get_hpd_status = lt8619_get_hpd_status,
};

int bridge_phy_lt8619_init(struct loonggpu_dc_bridge *dc_bridge)
{
	struct regmap *regmap;
	struct loonggpu_bridge_phy *lt8619_phy = bridge_phy_alloc(dc_bridge);
	struct loonggpu_dc_i2c *li2c = lt8619_phy->li2c;
	struct i2c_client *client = li2c->ddc_client;

	lt8619_phy->cfg_funcs = &lt8619_cfg_funcs;
	lt8619_phy->hpd_funcs = &lt8619_hpd_funcs;

	regmap = devm_regmap_init_i2c(client, &lt8619_regmap_config);
	if (IS_ERR(regmap)) {
		DRM_ERROR("Failed to regmap\n");
		return -EINVAL;
	}

	lt8619_phy->phy_regmap = regmap;
	lt8619_phy->regmap_cfg = NULL;

	lt8619_read_revision(lt8619_phy);
	lt8619_enable_sys_clk(lt8619_phy, true);
	lt8619_lvds_tx_clk_enable(lt8619_phy);

	return bridge_phy_init(lt8619_phy);
}
