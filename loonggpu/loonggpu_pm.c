		...
		char *buf)
{
	...
	gpu_res = dc_get_vbios_resource(adev->dc->vbios, 0, LOONGGPU_RESOURCE_GPU);
...
static ssize_t loonggpu_read_level(const char *buf, size_t count, uint32_t max_level)
{
	int ret;
	char *tmp;
	char **str;
		...
		size_t count)
{
	...
	if (adev->family_type == CHIP_LG100)
		loonggpu_cmd_exec(adev, GSCMD(GSCMD_FREQ, STATIC_FREQ), level, 0);
	else if (adev->family_type == CHIP_LG200 ||
		 adev->family_type == CHIP_LG210) {
		loonggpu_cmd_exec(adev, LG2XX_ICMD32(LG2XX_ICMD32_MOP_FREQ, 
				LG2XX_ICMD32_SOP_FREQ_UFRQ), level, 0);
	} else
...
static DEVICE_ATTR(gpu_clk, S_IRUGO | S_IWUSR,
		loonggpu_get_gpu_clk,
		loonggpu_set_gpu_clk);
