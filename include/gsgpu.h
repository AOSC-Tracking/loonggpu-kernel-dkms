...
extern int gsgpu_panel_cfg_clk_pol;
extern int gsgpu_panel_cfg_de_pol;
...
gsgpu_gem_prime_import_sg_table(struct drm_device *dev,
				 ...
				 struct sg_table *sg);
struct dma_buf *gsgpu_gem_prime_export(struct drm_gem_object *gobj, int flags);
struct drm_gem_object *gsgpu_gem_prime_import(struct drm_device *dev,
					    struct dma_buf *dma_buf);
#if defined(LG_DRM_DRIVER_HAS_GEM_PRIME_RES_OBJ)
lg_dma_resv_t *gsgpu_gem_prime_res_obj(struct drm_gem_object *);
#endif
...
static inline struct gsgpu_device *gsgpu_ttm_adev(
